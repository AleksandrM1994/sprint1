...
func (suite *EndpointsTestSuite) Test_SaveUrlHandler_Success(t *testing.T) {
	type Request struct {
		method string
		url    string
		body   string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			suite.repo.EXPECT().CreateURL(
				...
				"39e529f3-7947-4d3b-aee4-d49a3a757c0f").Return(nil).Times(1)
...
func (suite *EndpointsTestSuite) Test_SaveUrlHandler_ConflictError(t *testing.T) {
	type Request struct {
		method string
		url    string
		body   string
	...
	tests := []struct {
		...
		{
			name: "Test Save URL conflict error",
			request: Request{
				method: http.MethodPost,
				url:    "http://localhost:8080/",
				body:   "https://practicum.yandex.ru",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			suite.repo.EXPECT().CreateURL(
				...
				"8a9923515b44",
				"https://practicum.yandex.ru",
				"39e529f3-7947-4d3b-aee4-d49a3a757c0f").Return(custom_errs.ErrUniqueViolation).Times(1)
			suite.repo.EXPECT().GetURLByShortURL(gomock.Any(), "8a9923515b44").Return(&repository.URL{
				ID:          1,
				ShortURL:    "8a9923515b44",
				OriginalURL: "https://practicum.yandex.ru",
...
func (suite *EndpointsTestSuite) Test_SaveUrlHandler_CreateUrlError(t *testing.T) {
	type Request struct {
		method string
		url    string
		body   string
	...
	tests := []struct {
		...
		{
			name: "Test Save URL create url error",
			request: Request{
				method: http.MethodPost,
				url:    "http://localhost:8080/",
				body:   "https://practicum.yandex.ru",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			suite.repo.EXPECT().CreateURL(
				...
				"8a9923515b44",
				"https://practicum.yandex.ru",
				"39e529f3-7947-4d3b-aee4-d49a3a757c0f").Return(errors.New("some error")).Times(1)
...
func (suite *EndpointsTestSuite) Test_SaveUrlHandler_BadRequestError(t *testing.T) {
	type Request struct {
		method string
		url    string
		body   string
