...
func (suite *EndpointsTestSuite) Test_AuthUserHandler_UserNoFound(t *testing.T) {
	type Request struct {
		method string
		url    string
		body   *endpoints.AuthUserRequest
	...
	tests := []struct {
		...
		{
			name: "Test auth user error, user no found",
			request: Request{
				method: http.MethodPost,
				url:    "http://localhost:8080/api/user/auth",
				body: &endpoints.AuthUserRequest{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			suite.repo.EXPECT().GetUser(
				...
				"Ho8bmla6ULoW2wIaJj0jjj5wKXh/Wtbl5IUmKXaW/3U=",
				"Ho8bmla6ULoW2wIaJj0jjj5wKXh/Wtbl5IUmKXaW/3U=",
			).Return(nil, custom_errs.ErrNotFound).Times(1)
